...
import {useCallsState} from '@app/products/calls/state';
...
export const CallsCustomMessage = ({post, currentUser, author, isMilitaryTime, teammateNameDisplay, currentChannelName, callChannelName}: Props) => {
    const intl = useIntl();
    ...
    const callsState = useCallsState();
    const myCurrentCall = callsState.joined ? callsState.calls[callsState.joined] : null;
    const call = callsState.calls[post.channelId] || null;
    const confirmToJoin = Boolean(myCurrentCall && call && myCurrentCall.channelId !== call.channelId);
    const alreadyInTheCall = Boolean(myCurrentCall && call && myCurrentCall.channelId === call.channelId);
    ...
    const joinHandler = () => {
        ...
        leaveAndJoinWithAlert(intl, serverUrl, post.channelId, callChannelName, currentChannelName, confirmToJoin, joinCall);
    };
